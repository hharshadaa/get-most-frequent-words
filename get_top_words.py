...
def getTopWords(words: 'List[str]', K: int) -> 'List[str]':
    ...
    for i in range(len(words)): #iterate through words using length amount of times
      words[i] = words[i].lower() #turns all the words lowercase 

    no_stopwords = [] #empty list (will have words that are no in STOPWORDS list)
    for i in words: #iterate through words
     if i not in STOPWORDS: #if the current index is seen not in STOPWORDS...
        no_stopwords.append(i) #add it to the no_stopwords 

    sorted_list = sorted(no_stopwords) #sort list alphabetically to elimate accordingly
 
    counts = collections.Counter(sorted_list) #counts the frequency of words in no stop words
    # ^^ uses .Counter() from collections library 
    common_words = counts.most_common(K) #outputs nested list with tuples in regards to K (word amount)
    print(common_words)

    just_words = [] #empty list to store just the words not numbers in tuple
    for i, j in common_words: #iterate through common_words list and tuples
      just_words.append(i) #append the word to just_words
    return just_words #return the final words
